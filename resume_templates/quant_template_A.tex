...
\begin{supertabular}{p{3.4cm} p{13.9cm}}
	...
	\textsc{Computing}			& \begin{enumerate*}[label =$\diamond$, itemjoin={\newline}]
																\item \small \textsc{Python, C++, Java, R, Stata, Spark, Hadoop, SQL}
																\item \small \textsc{HTML, CSS, PHP, JSON, Node.js, Flask, Django, Shiny}
...
\begin{supertabular}{l p{0.55cm} p{13.9cm}}
	...
	\small\textsc{}							&& \small	\emph{Graduate Research Assistant}, \small Department of GADEPARTMENT \\
															&& \begin{enumerate*}[label =$\diamond$,itemjoin={\newline}]
																	\item \footnotesize Developed an enhanced Bayesian algorithm wrapper with $\mathbf{Rcpp}$ (using $\mathbf{C++}$ in $\mathbf{R}$) to improve computational scaling for petacomputing. Supervised junior statisticians and programmers.
	...
	\textsc{Jun-Sep 2018}	&& \textsc{INTERNSHIP1}, IC1_CITYSTATE \\
												...
												&& \begin{enumerate*}[label =$\diamond$, itemjoin={\newline}]
													\item \footnotesize Designed original research to evaluate quantitative portforlio trading models and optimize $\alpha$.
													\item \footnotesize Applied Bayesian Makov Chain Monte Carlo (MCMC) models in $\mathbf{R}$ to simulate portfolio returns.	\end{enumerate*} \\
	\multicolumn{2}{c}{} \\

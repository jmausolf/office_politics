...
\begin{supertabular}{p{13.9cm}>{\raggedleft\arraybackslash}p{3.4cm}}
	...
	\begin{enumerate*}[label =$\circ$,itemjoin={\newline}]
		\item \footnotesize Authored a mathematical proof for nonparametric bound estimation and wrote a \emph{C++} library to demonstrate utility for deep reinforcement learning. 
	\end{enumerate*} & \\ 
...
\begin{supertabular}{p{8.65cm} p{8.65cm}}
	...
	& \begin{enumerate*}[label =$\circ$, itemjoin={\newline}]
	 	\item \small \emph{Google Office}: \footnotesize Sheets, Documents, Slides, Gmail
	 	\item \small \emph{Microsoft Office}: \footnotesize Excel, Word, PowerPoint, Outlook, Acess	
	 	\item \small \emph{Adobe Office}: \footnotesize Acrobat Xi, Photoshop, Illustrator
	\end{enumerate*} \\

...
#df1_fullb <- inner_join(df1_full, dft)

df1_fullb <- bind_cols(df1_full, dft)
...
#   add_tally(callback_binary, name = "total_callbacks") %>%
#   add_count(pm_var, name = "total_applications") %>%
#   summarise(total_callbacks = unique(total_callbacks),
#             total_applications = unique(total_applications),
#             smean = mean(callback_binary, na.rm = TRUE),
#             ssd = sd(callback_binary, na.rm = TRUE),
#             count = n()) %>%
#   mutate(se = ssd / sqrt(count),
#          lower.ci.mpg = smean - qt(1 - (0.05 / 2), count - 1) * se,
#          upper.ci.mpg = smean + qt(1 - (0.05 / 2), count - 1) * se)
# 
# df1 <- dfec_edit %>%
#   #filter(pair_callback_bin != 2) %>%
#   select(callback_binary, pm_var, partyX) %>%
...
# 
# df1a_full <- bind_cols(df1a, df1_err)
# 
...
make_bar_app_match_party <- function(df_in){
  ...
  g <- ggplot(df_in, aes(pm_var, proportion_callback, fill=pm_var)) +
    ...
    geom_errorbar(aes(ymin=li1, ymax=ui2), width=0.4) + 
...
make_bar_app_match_party(df1_fullb)
